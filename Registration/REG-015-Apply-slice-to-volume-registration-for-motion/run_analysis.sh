...
echo "=========================================="
echo "REG-015: Apply slice-to-volume registration for motion correction in Mixed Gambles"
...
echo ""
echo "Step 4: Running analysis for REG-015..."
python3 << 'PYEOF'
import os
import sys
...
if [ -f "${EVIDENCE_DIR}/slice_transforms.txt" ] || [ -d "${EVIDENCE_DIR}/slice_transforms.txt" ]; then
    echo "✓ Found: slice_transforms.txt"
else
    echo "⚠ Not found: slice_transforms.txt (may need task-specific implementation)"
fi
...
# REG-015 Evidence: Apply slice-to-volume registration for motion correction in Mixed Gambles
