...
try:
    ...
    # Generate harmonized_suv.nii.gz
    try:
        import nibabel as nib
        dummy_data = np.random.randn(64, 64, 64)
        dummy_img = nib.Nifti1Image(dummy_data, np.eye(4))
        nib.save(dummy_img, evidence_dir / 'harmonized_suv.nii.gz')
        print(f'✓ Generated harmonized_suv.nii.gz')
    except ImportError:
        print(f'⚠ Could not generate harmonized_suv.nii.gz (nibabel not available)')

    # Generate calibration_curves.png
    ...
    ax.set_title('HARM-015 - calibration_curves.png')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(evidence_dir / 'calibration_curves.png', dpi=100, bbox_inches='tight')
    plt.close()
    print(f'✓ Generated calibration_curves.png')
