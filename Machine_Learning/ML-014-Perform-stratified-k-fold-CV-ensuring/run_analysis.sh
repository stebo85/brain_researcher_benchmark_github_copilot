...
echo "=========================================="
echo "ML-014: Perform stratified k-fold CV ensuring balanced classes in ADHD dataset"
...
echo ""
echo "Step 4: Running analysis for ML-014..."
python3 << 'PYEOF'
import os
import sys
...
if [ -f "${EVIDENCE_DIR}/cv_fold_indices.json" ] || [ -d "${EVIDENCE_DIR}/cv_fold_indices.json" ]; then
    echo "✓ Found: cv_fold_indices.json"
else
    echo "⚠ Not found: cv_fold_indices.json (may need task-specific implementation)"
fi

if [ -f "${EVIDENCE_DIR}/cv_scores_per_fold.csv" ] || [ -d "${EVIDENCE_DIR}/cv_scores_per_fold.csv" ]; then
    echo "✓ Found: cv_scores_per_fold.csv"
else
    echo "⚠ Not found: cv_scores_per_fold.csv (may need task-specific implementation)"
fi
...
# ML-014 Evidence: Perform stratified k-fold CV ensuring balanced classes in ADHD dataset
...
1. **cv_fold_indices.json**
2. **cv_scores_per_fold.csv**
